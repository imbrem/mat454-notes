...
\[\omega = f(z)dz = \left(\omega_1 + \left(\frac{c_1}{z} + \frac{c_2}{z^2} + ...\right)\right)dz\]
...
\[\Res_a(\omega) = \frac{1}{2\pi i}\int_\gamma\omega = c_1\]
...
\[\int_\Gamma\omega = 2\pi i \sum_{a \in E \cap K}\Res_a(\omega)\]
\end{theorem}
...
A \textbf{covering space} of a complex manifold \(Y\) is an unramified Riemann surface such that every \(h \in Y\) has a neighborhood \(V\) such that \(\varphi^{-1}(V)\) is a disjoint union of open \(U_i \subset X\), with each mapped isomorphically onto \(V\) by \(\varphi\).
\end{definition}
...
An example of a Riemann surface is given by, taking \(Y = \mbb{C}^*\) and \(X = \mbb{C}\), the function \(\varphi = z \mapsto e^t\).

In fact, in the above example, \(\varphi: X \to Y\) is a covering space of \(Y\). In particular, each branch of \(\log z\) defines an isomorphism from \(V\) to an open set in \(\mbb{C}\).

\begin{theorem}
Any connected open set in \(\mbb{C}\) (or any connected complex manifold \(Y\)) has a simply connected covering space.
\end{theorem}

In the example, \(z = e^t\) is a local coordinate in a neighborhood of any point of \(X\) (i.e. a holomorphic function on \(X\) can be expressed \underline{locally} as a holomorphic function of \(z\), but not in general \underline{globally}). For example, \(t\) is a holomorphic function on \(X\) in a neighborhood of each point, as it is a brnach of \(\log z\). We make \(\log z\) single valued by lifting it to the Riemann surface.

\TODO{clarify}

As another example, we consider a Riemann surface to make the multi-valued function \(y = (1 - x^3)^{1/3}\) single-valued: we define
\[X = \{(x, y) \in \mbb{C}^2 : x^3 + y^3 = 1\}, \qquad \varphi: X \to \mbb{C} = (x, y) \mapsto x\]
\(X\) is a manifold. Local coordinates at a point \((x_0, y_0) \in X\)...
