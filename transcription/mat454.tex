...
\end{equation}
Now, remember that \(x'\) is a local coordinate, and \(t'\) can be written in terms of \(x'\) by Equation \ref{equation:tprime_from_xprime_homo}. So, we can write \(dt'\) in terms of \(dx'\) using this expression to obtain
\begin{equation}
  dx' - \frac{12x'^2 + ...}{4x'^2 + ...}dx' = -2dx'(1 + g(x'))
\end{equation}
