...
\begin{enumerate}
  ...
  Then, the closure of our new space \(X\) given by \(x^3 + y^3 = 1\) is given by homogeneizing it's equation, because, in terms of the homogeneous coordinates, \([x, y, 1]\) is like \([x/z, y/z, z]\), so we homogeneize the equation by making that substitution, and we get a homogeneous equation
  \begin{equation}
    x^3 + y^3 = z^3
  \end{equation}
  ...
  \begin{equation}
    \{[x, y, 0] : x^3 + y^3 = 0\}
  \end{equation}
  In this case we can fix \(y\), looking at \(\frac{x}{y}\), and we get three points at infinity, given by \([i, -1, 0]\), \([j, -1, 0]\) and \([j^2, -1, 0]\) where \(j = e^{2\pi i/3}\).
