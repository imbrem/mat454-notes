...
These notes are based off a combination of my class notes and handwritten notes generously provided by Professor Edward Bierstone. While I made my best efforts to ensure their contents are correct, there may be a variety of errors and inconsistencies, all of which are my own. The course was taught using Ahlfors' \textit{Complex Analysis} \cite{ahlfors}. You may also find the notes for an earlier version of this course at \cite{bierstone_lectures} helpful.
...
\begin{equation}
  df = \frac{1}{2}\left(\prt{f}{x} - i\prt{f}{y}\right)dz
  + \frac{1}{2}\left(\prt{f}{x} + i\prt{f}{y}\right)d\bar{z}
\end{equation}
...
We now proceed to some basic properties of differential forms. We're going to be a bit informal in these notes, but a more precise definition can be obtained from, e.g., Spivak's \textit{Calculus on Manifolds} \cite{spivak_com}.
\begin{definition}[Differential Form]
For our purposes, we will define a differential form \(\omega\) as the abstract expression
\begin{equation}
  \omega = Pdx + Qdy
\end{equation}
where \(P, Q\) are continuous and either real or complex valued, in which case the form is itself real or complex valued.
\end{definition}
...
As a corollary, we can also prove the converse of Cauchy's theorem: Morera's theorem.
\begin{corollary}[Morera's Theorem]
  If \(f(z)\) is continuous in \(\Omega\) and \(df = f(z)dz\) is closed, then \(f(z)\) is holomorphic.
\end{corollary}
\begin{proof}
 We have that, locally, \(f(z)\) has a primitive \(g(z)\): hence,
 \begin{equation}
  f(z)dz = dg = \prt{g}{t}dz + \prt{g}{\ol{z}}d\ol{z} \implies \prt{g}{\ol{z}} = 0 \implies g \ \text{holomorphic} \implies f = \frac{dg}{dz} \ \text{holomorphic}
 \end{equation}
...
\end{definition}
The winding number is invariant under a homotopy of \(\gamma\) not passing through \(a\). As a function of \(a\), \(w(\gamma, a)\) is continuous on connected components of the complement of \(\gamma(S^1)\) (which we will just write as \(\gamma\) for convenence when confusion is unlikely).
...
\subsection{Integral Formulas for Taylor Coefficients and Cauchy's Inequalities}
...
\end{equation}
Now let \(\gamma(\theta) = re^{i\theta}\) for some \(r < R\). We have by Cauchy's integral formula that
\begin{equation}
  ...
  \label{equation:taylor_to_fourier}
\end{equation}
The coefficients we obtain by substituting \(\zeta = re^{i\theta}\), \(d\zeta = ire^{i\theta}d\theta\) into Equation \ref{equation:taylor_to_fourier},
\begin{equation}
  f(re^{i\theta}) = \sum_{n = 0}^\infty a_nr^ne^{i\pi\theta}, \qquad a_nr^n = \frac{1}{2\pi}\int_0^{2\pi}e^{-in\theta}f(re^{i\theta})d\theta
\end{equation}
are called the \textbf{Fourier coefficients of \(f\)}. Note that we can also derive these as follows:
\begin{equation}
  \frac{1}{\zeta - z} = \frac{1}{\zeta}\frac{\zeta}{z - \zeta}
  = \frac{1}{\zeta}\left(1 - \frac{z}{\zeta}\right)^{-1}
  = \frac{1}{\zeta}\sum_{k = 0}^\infty\frac{\zeta^k}{z^k}
  \implies f(z) = \frac{1}{2\pi i}\int_\gamma\sum_{k = 0}^\infty z^n\frac{f(\zeta)}{\zeta^{n + 1}}d\zeta
\end{equation}
...
\end{equation}
So harmonic functions satisfy the MVP and MMP, and conversely, a continuous function in an open set \(\Omega \subseteq \mbb{C}\) satisfying the MVP is harmonic. Just a couple of words as to why this is true, as this is really something that you should review: this comes from the solution to what's called the \textbf{Dirichlet problem for a disk}: given any continuous function \(f\) on the boundary of a disk \(|z| < r\), can you extend it to a continous function \(F\) on the whole disk which is harmonic on the interior?

\begin{theorem}[Dirichlet Problem]
  The Dirichlet problem for a closed disk has a \underline{unique} solution.
\end{theorem}
\begin{proof}
  \TODO{this}
\end{proof}
...
\begin{corollary}
  Any continuous function in an open set \(\Omega\) satisfying the Mean Value Property is harmonic.
\end{corollary}
\begin{proof}
  \TODO{this}
\end{proof}
...
\end{equation}
\end{definition}
Note that the \(P(z)\) converges when \(r < |z|\), whereas \(R(z)\) converges when \(|z| < R\). This actually comes from Cauchy's theorem, just in the case of a convergent power series expansion of a holomorphic function. So this is from Cauchy's integral formula:
\begin{equation}
  f(z) =
    \frac{1}{2\pi i}\int_{\gamma_1}\frac{f(\xi)}{\xi - z}d\xi
    - \frac{1}{2\pi i}\int_{\gamma_2}\frac{f(\xi)}{\xi - z}d\xi
...
\end{definition}
It turns out that we can show that, if \(f\) is holomorphic on a punctured disc around \(0\), extending \(f\) to the disc \(|z| < R\) is possible if and only if \(f(z)\) is bounded in a neighborhood of zero: writing out the Laurent series, we have
\begin{equation}
  \forall \rho < |z| < R, f(z) = \sum_{n = -\infty}^\infty a_nz^n \implies \forall \rho < r < R, f(re^{i\theta}) = \sum_{n = -\infty}^\infty a_nr^ne^{-in\theta}
\end{equation}
with
\begin{equation}
  \forall n \in \ints, a_nr^n = \frac{1}{2\pi}\int_0^{2\pi}e^{-in\theta}f(re^{i\theta})d\theta \implies |a_n| \leq \frac{M(r)}{r^n}
\end{equation}
where \(M(r)\) denotes the upper bound for \(|f(z)|\), \(|z| = r\). If \(f\) is bounded in the punctured disc, then \(|a_n| \leq \frac{M}{r^n}\) for all small \(r\), implying that \(a_n = 0\) for all \(n < 0\). This leads to the Weierstrass theorem:
...
\TODO{this}
\end{proof}
In fact, we'll show later on in the course that we can do a lot better: see Theorem \ref{theorem:big_picard}.
...
\end{definition}
Our goal is to eventually compute the residue of a meromorphic function \(f\) around all its poles. While we can use the above definition for poles \(a \in \mbb{C}\), we must also consider the case of functions like \(f(z) = z\), which have a ``\textbf{pole at infinity}". We do so with the following definition
...
\end{definition}
We may now state one of the most important practical results in complex analysis: the Residue theorem
...
\begin{equation}
  \forall \ \text{compact} \ K \subset \Omega, \forall \epsilon > 0,
    V(K, \epsilon) = \{f: \forall z \in K, |f(z)| < \epsilon\}
\end{equation}
...
\begin{equation}
  d(f) = \sum_{i = 1}^\infty\frac{1}{2^i}\min\{1, M_i(f)\},
  \quad M_i(f) = \max_{K_i}f
\end{equation}
\end{definition}
This is a metric as
\begin{equation}
...
\begin{definition}[Holomorphic function of several variables]
Let \(\Omega \subset \mbb{C}^n\) be open. We say \(f: \mbb{C}^n \to \mbb{C}\) is \textbf{holomorphic} if it is \(\mc{C}^1\) and
\begin{equation}
  df = \sum\prt{f}{z_j}dz_j \iff \forall j, \prt{f}{\ol{z_j}} = 0
\end{equation}
It is not hard to show that a several-valued variable being holomorphic is equivalent to:
\begin{enumerate}

  \item \(f\) is continuous and separately holomorphic functions in each variable (use the Cauchy integral).

  \item \(f\) is analytic

\end{enumerate}
In fact, separately holomorphic in each variable on its own implies holomorphic (though this is hard to show).
\end{definition}
Similarly to in multivariable calculus, we have a form of the implicit function theorem for complex functions:
\begin{theorem}[Implicit function theorem]
  Let \(\Omega \subset \mbb{C}^2\) be open and \(f: \Omega \to \mbb{C}\) be holomorphic.
  Assume \(f(x_0, y_0) = 0\) for some \((x_0, y_0) \in \Omega\), and \(\prt{f}{y}(x_0, y_0) \neq 0\). Then we can solve locally for \(y\) as a holomorphic function of \(x\).
\end{theorem}
\begin{proof}
  \TODO{this}
\end{proof}

\subsection{Theorem of Mittag-Leffer (1877)}

Recall that a meromorphic function in \(S^2\) is rational (see Theorem \ref{theorem:meromorphic_rational}, proved using the partial fraction decomposition). We have that \(\tan z\) and \(\sec z\) are meromorphic in \(\mbb{C}\).
\begin{theorem}[Mittag-Leffer]
Let \(b_k \in \mbb{C}\) such that \(\lim_{k \to \infty}b_k = \infty\), and \(P_k(z)\) be a polynomial without a constant term (i.e. with \(P_k(0) = 0\)). Then there exists a meromorphic function on \(\mbb{C}\) with poles \(b_k\) and corresponding principal parts \(P_k\left(\frac{1}{z - b_k}\right)\). Furthermore, the most general meromorphic function with these poles and prime parts can be written
\begin{equation}
  f(z) = \sum_k\left(P_k\left(\frac{1}{z - b_k}\right) - p_k(z)\right) + g(z)
\end{equation}
where \(g(z)\) is holomorphic in \(\mbb{C}\) and \(p_k(z)\) are polynomials such that the series converges uniformly and absolutely on compact subsets of \(\mbb{C}\).
\end{theorem}
\begin{proof}
\TODO{this}
\end{proof}
...
Consider the expression, for some \(b_n \in \mbb{C}\),
\begin{equation}
  P = \prod_{n = 1}^\infty b_n
  \label{equation:infinite_product}
\end{equation}
The obvious way to define it's value would be as a limit of partial products:
\begin{equation}
  p_n = \prod_{k = 1}^n b_k, \qquad P = \lim_{n \to \infty}p_n
\end{equation}
if the limit exists and is nonzero. This, however, is too restrictive: we'll say instead that \(P\) in equation \ref{equation:infinite_product} \textbf{converges} if all but \textit{finitely many} factors are nonzero, and the partial products formed by the nonzero factors have a nonzero limit. Hence, we have that, \textit{omitting zero factors},
\begin{equation}
  \prod_{n = 1}^\infty b_n \ \text{converges} \implies \lim_{n \to \infty}b_n = \lim-{n \to \infty}\frac{p_n}{p_{n - 1}} = 1
\end{equation}
Therefore, we can instead write the above product as
\begin{equation}
  P = \prod_{n = 1}^\infty(1 + a_n), \qquad \lim_{n \to \infty} a_n = 0
\end{equation}
Hence, choosing a prime branch of log, which is defined for \(n\) large enough, we can take partial sums
\[s_n = \sum_{n = 1}^k\log(1 + a_n) \implies p_n = e^{s_n}\]
Hence, if \(s_n\) converges, \(p_n\) converges, and if \(p_n\) converges, \(s_n\) converges. The only problem is the convergence of the imaginary part of \(S_n\). To address this, fix \(\log p = \log|p| + i\arg p\). Set
\begin{equation}
  \log p_n = \log |p_n| + i\arg p_n, \qquad \arg p_n \in (\arg p - \pi, \arg p + \pi)
\end{equation}
Then \(s_n = \log p_n + 2\pi ik_n\) for \(k_n \in \ints\). Hence, we have
\begin{equation}
  \log(1 + a_{n + 1}) = s_{n + 1} - s_n = \log p_{n + 1} - \log p_n + 2\pi i(k_{n + 1} - k_n)
\end{equation}
For \(n\) large enough, \(|\arg(1 + a_{n + 1})|\) and \(|\arg(p_n) - \arg(p)|\) are all less than \(\frac{2\pi}{3}\), and therefore \(|k_{n + 1} - k_n| < 1\), implying, as these are integers, \(k_{n + 1} = k_n\). Hence, for \(n\) large enough, \(k_n = k\) is eventually constant. Hence,
\begin{equation}
  \lim_{n \to \infty}s_n = \log p + 2\pi ik
\end{equation}
\begin{definition}
We say that \(P = \prod_{k = 1}^\infty(1 + a_n)\) \textbf{converges} if \(\sum\log(1 + a_n)\) converges absolutely.
\end{definition}
This is equivalent to absolute convergence of \(\sum a_n\):
\TODO{why}

\begin{theorem}
  Assume \(f_n\) is holomorphic in \(\Omega\), and \(\prod_{n = 1}^\infty f_n\) converges uniformly and absolutely on compact sets of \(\Omega\). Then
  \begin{enumerate}

    \item \(f = \prod f_n\) is holomorphic in \(\Omega\)

    \item The set of zeros of \(f\) is equal to the union of the zero sets of \(f_n\). The multiplicity of the each zero of \(f\) is equal to the sum of the multiplicities for each \(f_n\) (which is well defined as only finitely many \(f_n\) may be zero at a given point).

    \item The series of meromorphic functions \(\sum_{n = 1}^\infty\frac{f_n'}{f_n}\) converges uniformly and absolutely on compact subsets of \(\Omega\), and the sum is \(\frac{f'}{f}\).
...
\begin{equation}
  S = T \circ (T^{-1} \circ S),
  \quad (T^{-1} \circ S)(z_0) = z_0
  \implies T^{-1} \circ S \in G \implies S \in G
\end{equation}
...
\begin{equation}
  S(z) = e^{i\theta}\frac{z - z_0}{1 - \bar{z_0}z},
  \qquad \text{where} \ z_0 = T(0), \quad \theta = \Arg T'(z_0)
\end{equation}
...
Let's consider an alternative proof. We can phrase little Picard's theorem as ``if an entire function misses two values then it is constant." This should remind you of Liouville's theorem. There's a very powerful theorem in complex analysis that tells you there are three simply connected surfaces: the sphere, the plane and the unit disc. Anything covered by the disc is called ``hyperbolic." Now, the covering space of \(X = \mbb{C} / \{x, y\}\) for \(x \neq y\) is the disc, which is biholomorphic to the upper half-plane. So if \(f\) is entire and misses two points, we can write it as \(f: \mbb{C} \to X\), and hence it lifts to a holomorphic map to the covering space \(f: \mbb{C} \to D\), and Liouville's theorem tells us that must be constant.
