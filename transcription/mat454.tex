...
\begin{equation}\int_\gamma\omega = \int_a^bf(t)dt, \quad f(t) = P(x(t), y(t))x'(t) + Q(x(t), y(t))y'(t) = \gamma^*\omega\end{equation}
\end{theorem}
Note we can define \(\gamma^*\) recursively by
\begin{equation}\gamma^*(P) = P\circ \gamma, \quad \gamma^*(dx) = d(x \circ \gamma) = x'(t)dt, \quad \gamma^*(dy) = d(y \circ \gamma) = y'(t)dt\end{equation}
...
\label{example:et_riemann_surface}
...
In the example \hyperref[et_riemann_surface]{above}, \(z = e^t\) is a local coordinate in a neighborhood of any point of \(X\) (i.e. a holomorphic function on \(X\) can be expressed \underline{locally} as a holomorphic function of \(z\), but not in general \underline{globally}). For example, \(t\) is a holomorphic function on \(X\) in a neighborhood of each point, as it is a branch of \(\log z\). In other words, ``we make \(\log z\) single valued by lifting it to the Riemann surface."
...
\begin{equation}X = \{(x, y) \in \mbb{C}^2 : x^3 + y^3 = 1\}, \qquad \varphi: X \to \mbb{C} = (x, y) \mapsto x\end{equation}
...
\begin{equation}X = \{(x, y) : y^2 = p(x)\}\end{equation}
