...
A \textbf{complex structure} on a manifold \(M\) is given by an open cover of \(\{U_i\}\) of \(M\) and a set of \textbf{coordinate charts} \(\varphi_i: U_i \to \mbb{C}\) such that each composition \(\varphi_i \circ \varphi_j^{-1}: \varphi_j(U_j) \to \mbb{C}\) is holomorphic.
\end{definition}
...
A \textbf{complex manifold} is a topological manifold \(M\) together with an equivalence class of complex structures
\end{definition}
