...
Assume \(f(z)\) is holomorphic in \(D = \{|z| < R\}\), and let \(\gamma(\theta) = re^{i\theta}\) for some \(r < R\). We have by Cauchy's integral formula that
\begin{equation}
...
\end{equation}
Writing the Taylor series, we obtain
\begin{equation}
  f(z) = \sum_{n = 0}^\infty a_nz^n, \qquad a_n = \frac{f^{(n)}(0)}{n!} = \frac{1}{2\pi i}\int_\gamma\frac{f(\zeta)d\zeta}{\zeta^{n + 1}}
